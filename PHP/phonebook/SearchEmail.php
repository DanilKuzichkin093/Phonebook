...
function searchPhonesByName($query){
    global $phones;
    $result = array();
    foreach ($phones -> contact as $computer){
        if (substr(strtolower($computer -> name), 0, strlen($query))==strtolower($query))
...
function searchPhonesByPhone($query){
    global $phones;
    $result = array();
    foreach ($phones -> contact as $computer){
        if (substr(strtolower($computer -> phone), 0, strlen($query))==strtolower($query))
            array_push($result, $computer);
    }
    return $result;
}
function searchPhonesByEmail($query){
    global $phones;
    $result = array();
    foreach ($phones -> contact as $computer){
...
function searchPhonesByGender($query){
    global $phones;
    $result = array();
    foreach ($phones -> contact as $computer){
        if (substr(strtolower($computer -> gender), 0, strlen($query))==strtolower($query))
            array_push($result, $computer);
    }
    return $result;
}
...
<html>
    ...
    <body>			
        ...
        <form action="?" method="post">
            <h3>Search email:</h3> <input type="text" name="searchEmail" placeholder="Email"/>
        ...
        <thead>
            ...
            $result = searchPhonesByEmail($_POST["searchEmail"]);
            foreach($result as $arvuti) {
                ...
                echo "<td>".($arvuti -> surname)."</td>";
                echo "<td>".($arvuti -> phone)."</td>";
                echo "<td><ins>".($arvuti -> email)."</ins></td>";
				echo "<td>".($arvuti -> gender)."</td>";
                echo "</tr>";
                }
